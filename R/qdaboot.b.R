...
QDABOOTClass <- if (requireNamespace('jmvcore')) R6::R6Class(
    ...
    private = list(
        .run = function() {
            ...
            if((is.null(self$options$prod))||(is.null(self$options$pane)))return()
            
            else if(is.null(self$options$senso))return()
                ...
                thresh_gui=self$options$thresh
                level_gui=self$options$level_search
                ...
                cex_gui=self$options$cex
                namepan_gui=self$options$name_box
                ...
                if((isTRUE(self$options$cor_tab_box_min))){
                    private$.corTab(res_pan,"min")
                }
                if((isTRUE(self$options$cor_tab_box_max))){
        ...
        .hotTab = function(res){
            ...
            for (i in 1:dim(hote)[1]){
                ...
                for (j in 1:dim(hote)[1]){
                    row[[as.character(colnames(hote)[j])]]=hote[,j][i]
        ...
        .plotSpace = function(image, ...){
            ...
            if(is.null(self$options$senso)) return()
            res_pan=image$state
            ...
            gg_graph <- gg_graph + labs(x = lab.x, y= lab.y,title = "Confidence ellipses for the mean points")
        ...
        .plotPane = function(image, ...){
            ...
            gg_graph = ggplot() +
                ...
                geom_hline(yintercept = 0,lty=2) + geom_vline(xintercept = 0,lty=2) +
                theme_light()+ theme(axis.title = element_text(hjust = 1, face = 2), plot.title = element_text(hjust = 0.5, face = 2))+
                labs(title = "Individual description", x = paste0("Dim ",coord_gui[1]," (",eig[coord_gui[1],2],"%)"), y= paste0("Dim ",coord_gui[2]," (",eig[coord_gui[2],2],"%)"))+
        ...
        .plotIndaxe = function(image, ...){
            ...
            ploti=plot.PCA(graxe,choix="ind",invisible="ind.sup",axes=coord_gui)
            print(ploti)
            TRUE
        },
        .plotVaraxe = function(image, ...){
            ...
            plotv=plot.PCA(graxe,choix="var",habillage="group",axes=coord_gui)
            print(plotv)
            TRUE
        }
    )
