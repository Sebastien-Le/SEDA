    ...
    private = list(
        .run = function() {
            ...
            if (is.null(self$options$actvars)) return()
                ...
                data=data.frame(self$data[,self$options$actvars])
                colnames(data)=self$options$actvars
                ...
                #création jeu de données textuel
                c1 <- rownames(data)
                c2 <- as.vector(data[,1])
                j <- 0
        ...
        .descfreq = function(res) {
            threshold=self$options$leveltext
        ...
        .printeigenTable = function(table){
            ...
            } #on crée les lignes du tableau, avec autant de lignes qu'il y a de dimensions 
        ...
        .plotitemvar = function(image, ...) {
            ...
            if (is.null(self$options$actvars)) return()
                ...
                # else 
                # if (modqualisup_gui==FALSE && modvar_gui==TRUE)
                # if (modvar_gui==TRUE)
                    plot=plot.MCA(res.mca, axes=c(abs, ord), choix="ind", invisible=c("ind", "quali.sup"), selectMod = paste(modality_gui),title = "Representation of the descriptions")
                # else
        ...
        .plotindiv = function(image, ...) {
            ...
            if (is.null(self$options$actvars)) return()
                ...
                plot=plot.MCA(res.mca, axes=c(abs, ord), choix="ind", invisible=c("var","quali.sup", "quanti.sup"),title = "Representation of the products")
        ...
        .plotvar = function(image, ...) {
            ...
            if (is.null(self$options$actvars)) return()
                ...
                plot=plot.MCA(res.mca, axes=c(abs, ord), choix="var", title = "Representation of the consumers")
        ...
        .populateDFTable= function(tab){
            ...
            for (i in 1:(dim(tab)[1])) {
                ...
                row[["vtest"]]=tab[,8][i]
                self$results$dfresgroup$dfres$setRow(rowNo=i, values = row) #Méthode setRow
            }
