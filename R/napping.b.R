    ...
    private = list(
        .run = function() {
            ...
            if (is.null(self$options$actvars)) return()
                ...
                private$.printTables(res.napping$global.pca, "coord")
                private$.printTables(res.napping$global.pca, "contrib")
                private$.printTables(res.napping$global.pca, "cos2")
        ...
        .mfa = function(data, nbgr, qtsup=NULL,qlsup=NULL){
            ...
            if (is.null(quantisup_gui) == FALSE && is.null(qualisup_gui)== TRUE){
                ...
                FactoMineR::MFA(data,group=c(rep(2,7)),type=rep("c",7),ncp=nFactors_gui,name.group=c(paste("S",1:nbgr,sep="")),graph=FALSE)
            }
        },
        ...
        .printTables = function(table, quoi){
            ...
            if (quoi=="coord") {
                quoivar=as.data.frame(rbind(table$var$cor,table$quanti.sup$cor))
        ...
        .plotindividus = function(image, ...){
            ...
            if (is.null(self$options$actvars)) return()
                ...
                plot=FactoMineR::plot.MFA(res.napping,axes=c(abs_gui, ord_gui), choix="ind", habillage = "none")
        ...
        .plotindividus2 = function(image, ...){
            ...
            if (is.null(self$options$actvars)) return()
                ...
                plot=FactoMineR::plot.MFA(res.napping,axes=c(abs_gui, ord_gui), choix="ind", invisible="quali.sup",habillage = "none")
        ...
        .plotindividus3 = function(image, ...){
            ...
            if (is.null(self$options$actvars)) return()
                ...
                if (is.null(self$options$qualisup)==FALSE){
                    plot=FactoMineR::plot.MFA(res.napping,axes=c(abs_gui, ord_gui), choix="ind", invisible="ind",habillage = "none")
                    
                }
                else {plot=plot(as.data.frame(c(0)),main="You don't have categorical variables in your analysisis")
                }
                print(plot)
                TRUE
            }
        ...
        .plotvariables = function(image, ...){
            ...
            if (is.null(self$options$actvars)) return()
                ...
                abs_gui=self$options$abs
                ord_gui=self$options$ord
                limcosvar_gui=self$options$limcosvar
        ...
        .plotvariables3 = function(image, ...){
            ...
            if (is.null(self$options$actvars)) return()
                ...
                if (is.null(self$options$quantisup)==FALSE){
                    plot=FactoMineR::plot.MFA(res.napping, choix="var",invisible="quanti", axes=c(abs_gui, ord_gui),habillage="group",col.hab = c("black"),lim.cos2.var = limcosvar_gui,graph.type = "ggplot" )
                    
                }
                else {plot(as.data.frame(c(0)),main="You don't have supplementary variables in your analysisis")
                }
                print(plot)
                TRUE
            }
        ...
        .plotgroups = function(image, ...){
            ...
            if (is.null(self$options$actvars)) return()
                ...
                if (is.null(self$options$quantisup)== FALSE && is.null(qualisup_gui)== TRUE){
                    plot=FactoMineR::plot.MFA(res.napping,axes=c(abs_gui, ord_gui),choix="group",habillage="none", col.hab=c(rep("black",nbgr),"red"))
                }
                else if (is.null(quantisup_gui) == TRUE && is.null(qualisup_gui)== FALSE){
                    plot=FactoMineR::plot.MFA(res.napping,axes=c(abs_gui, ord_gui),choix="group",habillage="none", col.hab=c(rep("black",nbgr),"green"))
                }
                else if (is.null(quantisup_gui) == FALSE && is.null(qualisup_gui)== FALSE){
                    plot=FactoMineR::plot.MFA(res.napping,axes=c(abs_gui, ord_gui),choix="group",habillage="none", col.hab=c(rep("black",nbgr),"red","green"))
                }
                else {
                    plot=FactoMineR::plot.MFA(res.napping,axes=c(abs_gui, ord_gui),choix="group",habillage="none", col.hab=c(rep("black",nbgr)))
